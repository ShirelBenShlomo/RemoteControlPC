    ...
    : mContext(nullptr), mPacket(nullptr), mFrame(nullptr), mWidth(width), mHeight(height), mSwsContext(nullptr), mDecodedRGB(nullptr)
{
    ...
    // Set up a SwsContext for converting YUV420P to RGB (optional)
...
uint8_t* H264Decoder::decode(uint8_t* iEncodedData, size_t iSize)
{
    ...
    // Convert from YUV420P to RGB (optional)
    if (mSwsContext)
    {
        // Allocate memory for RGB output (if not already allocated)
        if (!mDecodedRGB)
        {
        ...
        int dstStride[1] = { mWidth * 3 };  // RGB stride
        sws_scale(mSwsContext, mFrame->data, mFrame->linesize, 0, mHeight, dst, dstStride);
    }

    return mDecodedRGB ? mDecodedRGB : mFrame->data[0];  // Return RGB or YUV data
}
...
{
    return mFrame->linesize[0] * mHeight;  // Return YUV size
}