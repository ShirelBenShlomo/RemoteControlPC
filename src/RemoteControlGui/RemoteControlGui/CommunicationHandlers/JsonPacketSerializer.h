...
public:
	...
	static std::vector<unsigned char> serialize(const FileTransmissionRequest& fileTransmissionRequest, HCRYPTPROV hAESKey);
	static std::vector<unsigned char> serialize(const FileTransmissionResponse& fileTransmissionResponse, HCRYPTPROV hAESKey);

	static std::vector<unsigned char> serializeFileData(const std::vector<unsigned char>& data, HCRYPTPROV hAESKey);
private:
